 ...
 */
import { MODEL_SAMPLE_RATE } from '../core/config.js';
...
export class AudioStreamer {
    ...
    /**
     * Completely clears all audio data and resets the streamer to initial state
     * More thorough than stop() - resets all buffers and state without fade-out
     */
    clear() {
        console.info('Clearing all audio data and resetting AudioStreamer');
        ...
        for (const source of this.scheduledSources) {
            try {
                ...
                console.debug('Error stopping audio source during clear', { error: error });
            }
        }
        this.scheduledSources.clear();
        ...
        // Reset gain immediately (no fade-out for refresh)
        try {
            if (this.gainNode) {
            ...
            console.debug('Error resetting gain during clear', { error: error });
        }
        
        console.info('AudioStreamer cleared and reset to initial state');
    }
